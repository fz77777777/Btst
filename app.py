    ...
    if strategy_mode == "🔥 Stockbee Hot-Sector Scanner":
        ...
        setup_filter = st.selectbox(
            ...
            options=[
                "All Setups Combined", 
                "Fresh EP Breakout", 
                "Late EP / Consolidation (Strict 6%)", 
                "Pullback Near 10/20 EMA",
                "Trend Continuer / Follow-through (Post-EP)"
            ],
            index=0
        )
...
# ==========================================
# EXTRA-STABLE RECOVERY BATCH FETCH ENGINE
# ==========================================
...
def fetch_indian_data_matrix(tickers, tf):
    ...
    # Primary Master Batch Request
    df = yf.download(tickers, start=start_date, end=end_date, interval=yf_interval, group_by='ticker', progress=False)
    
    # Secondary Multi-Index Structure Clean-up
    cleaned_df = {}
    for ticker in tickers:
        try:
            if ticker in df.columns.levels[0] and not df[ticker].dropna().empty:
                cleaned_df[ticker] = df[ticker]
            else:
                # Fallback Individual Fetch Engine for missing mid/small caps (like PASUPTAC)
                fallback_data = yf.download(ticker, start=start_date, end=end_date, interval=yf_interval, progress=False)
                if not fallback_data.empty:
                    cleaned_df[ticker] = fallback_data
        except Exception:
            continue
    return cleaned_df
...
if st.button(f"🔍 EXECUTE ALL-MARKET {strategy_mode.upper()} ENGINE"):
        ...
        with st.spinner("Processing multi-timeframe structural matrices..."):
            ...
            for ticker in TICKER_LIST:
                try:
                    ...
                    # 1. Check for Fresh EP Breakout Condition
                    if row['Pct_Change'] >= min_gain and row['Volume'] >= (vol_multiplier * row['Vol_SMA']):
                        status = "🚨 FRESH EP BREAKOUT"
                    else:
                        # Scan back to see if an EP occurred within the last 15 days
                        has_recent_ep = False
                        ep_idx = -1
                        ...
                        if has_recent_ep:
                            ...
                            elif row['Pct_Change'] >= min_gain and current_close > df.iloc[ep_idx]['Close']:
                                # NEW CONDITION: Captures follow-through multi-day momentum explosions like PASUPTAC
                                status = f"🚀 TREND CONTINUER / POST-EP RALLY ({bars_since} Bars Ago)"
                                trigger_gain = row['Pct_Change']
                                trigger_vol = row['Volume'] / row['Vol_SMA'] if row['Vol_SMA'] > 0 else 1.0
                    ...
                    if status:
                        if setup_filter == "Fresh EP Breakout" and "FRESH" not in status:
                            continue
                        if setup_filter == "Late EP / Consolidation (Strict 6%)" and "CONSOLIDATION" not in status:
                            continue
                        if setup_filter == "Pullback Near 10/20 EMA" and "PULLBACK" not in status:
                            continue
                        if setup_filter == "Trend Continuer / Follow-through (Post-EP)" and "TREND CONTINUER" not in status:
                            continue
                        ...
                        scanned_pool.append({
                            ...
                            "Setup Status": status,
                            "Current / Base Gain": f"{round(trigger_gain, 2)}%",
                            "Volume Multiple": f"{round(trigger_vol, 2)}x",
    ...
    # -------------------------------------------------------------
    # STRATEGY MODE 2: 3:00 PM BTST ALPHA ENGINE
    # -------------------------------------------------------------
        ...
        with st.spinner("Extracting multi-asset closing data streams..."):
            ...
            # Base benchmark verification
            if 'RELIANCE.NS' in all_data:
                benchmark_ticker = 'RELIANCE.NS'
            else:
                benchmark_ticker = list(all_data.keys())[0] if all_data else None
                
            for ticker in TICKER_LIST:
                try:
                    if ticker not in all_data or not benchmark_ticker:
                        continue
                    df = all_data[ticker].dropna()
                    if len(df) < 100:
                        continue
                    ...
                    nifty_series = all_data[benchmark_ticker]['Close'].iloc[idx-30:idx+1].astype(float)
            ...
            for sec in sector_rs_ratio.keys():
                if len(sector_rs_ratio[sec]) > 3:
                    ...
                    elif m_ratio >= 1.0 and m_momentum > 0:
                        quadrant = "🔥 Leading Quadrant"
                    elif m_ratio >= 1.0 and m_momentum <= 0:
                        quadrant = "⏳ Weakening Quadrant"
                    else:
                        quadrant = "🔴 Lagging Quadrant"
            ...
            for ticker in TICKER_LIST:
                try:
                    ticker_sector = SECTOR_MAP.get(ticker, "Other")
                    if ticker_sector not in improving_sectors:
                        continue
                    ...
                    price_pass = min_gain <= today_gain <= 8.5
            ...
            if btst_pool:
                ...
                st.success(f"🚀 **BTST Past Lookup Complete!** Found **{len(btst_df)} Stocks**!")
                st.dataframe(btst_df, use_container_width=True)
            else:
                st.warning(f"⚠️ Selected Lookback ({historical_days} Days Ago) par koi bhi stock criteria qualify nahi kar paya.")
